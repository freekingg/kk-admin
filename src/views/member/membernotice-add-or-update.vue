...
<!--      <el-form-item :label="$t('标签')" prop="flagIds">
        <el-input v-model="dataForm.flagIds" :placeholder="$t('标签')"></el-input>
      </el-form-item>-->
<!--      <el-form-item :label="$t('会员等级')" prop="">
        <el-radio-group v-model="dataForm.levelId">
          <el-radio :label="0">{{ $t("0") }}</el-radio>
          <el-radio :label="1">{{ $t("1") }}</el-radio>
          <el-radio :label="2">{{ $t("2") }}</el-radio>
          <el-radio :label="3">{{ $t("3") }}</el-radio>
          <el-radio :label="4">{{ $t("4") }}</el-radio>
          <el-radio :label="5">{{ $t("5") }}</el-radio>
        </el-radio-group>
      </el-form-item>-->
      <el-form-item prop="deadlyDate" :label="$t('过期时间')">
        <el-date-picker v-model="dataForm.deadlyDate" type="datetime" format="YYYY-MM-DD HH:mm:ss" value-format="YYYY-MM-DD HH:mm:ss" :placeholder="$t('过期时间')"></el-date-picker>
      </el-form-item>
    </el-form>
...
const getInfo = (id: number) => {
  baseService.get(`/member/membernotice/${id}`).then((res) => {
...
const dataFormSubmitHandle = (status: number) => {
  dataFormRef.value.validate((valid: boolean) => {
    ...
    (!dataForm.id ? baseService.post : baseService.put)("/member/membernotice/", dataForm).then((res) => {
      ElMessage.success({
