<template>
  <el-dialog v-model="visible" :title="!dataForm.id ? $t('add') : $t('update')" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-form :model="dataForm" :rules="rules" ref="dataFormRef" @keyup.enter="dataFormSubmitHandle()" label-width="120px">
      ...
      <el-form-item label="接收内容" prop="content">
        <el-input v-model="dataForm.content" placeholder="接收内容"></el-input>
      </el-form-item>
...
const getInfo = (id: number) => {
  baseService.get("/member/membernoticeuser/" + id).then((res) => {
...
const dataFormSubmitHandle = () => {
  dataFormRef.value.validate((valid: boolean) => {
    ...
    (!dataForm.id ? baseService.post : baseService.put)("/member/membernoticeuser", dataForm).then((res) => {
      ElMessage.success({
