    ...
    <el-form :model="dataForm" :rules="rules" ref="dataFormRef" @keyup.enter="dataFormSubmitHandle()" label-width="120px">
      ...
      <el-form-item :label="$t('sms.platform')">
        <el-radio-group v-model="dataForm.platform">
          <el-radio :label="5">塞班云</el-radio>
          <el-radio :label="4">{{ $t("sms.platform1") }}</el-radio>
          <el-radio :label="2">{{ $t("sms.platform2") }}</el-radio>
          <el-radio :label="3">{{ $t("sms.platform3") }}</el-radio>
          <el-radio :label="1">云片</el-radio>
        </el-radio-group>
      </el-form-item>
      <template v-if="+dataForm.platform === 4">
      ...
      <template v-if="+dataForm.platform === 1">
        <el-form-item prop="config.yunpianApiKey" label="云片ApiKey">
          <el-input v-model="dataForm.config.yunpianApiKey" placeholder="云片ApiKey"></el-input>
        </el-form-item>
        <el-form-item prop="config.yunpianTemplate" label="云片模板">
          <el-input v-model="dataForm.config.yunpianTemplate" placeholder="云片模板"></el-input>
        </el-form-item>
      </template>
      <template v-if="+dataForm.platform === 5">
        <el-form-item prop="config.subAppId" label="塞班云AppId">
          <el-input v-model="dataForm.config.subAppId" placeholder="塞班云AppId"></el-input>
        </el-form-item>
        <el-form-item prop="config.subApiKey" label="塞班云ApiKey">
          <el-input v-model="dataForm.config.subApiKey" placeholder="塞班云ApiKey"></el-input>
        </el-form-item>
        <el-form-item prop="config.subTemplate" label="赛白云模板">
          <el-input v-model="dataForm.config.subTemplate" placeholder="赛白云模板"></el-input>
        </el-form-item>
      </template>
    </el-form>
