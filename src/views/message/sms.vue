    ...
    <el-form :inline="true" :model="state.dataForm" @keyup.enter="state.getDataList()">
      <el-form-item>
        <el-select v-model="state.dataForm.platform" :placeholder="$t('sms.platform')" clearable>
          <el-option label="云片" :value="1"></el-option>
          <el-option label="塞班云" :value="5"></el-option>
          <el-option :label="$t('sms.platform2')" :value="2"></el-option>
          <el-option :label="$t('sms.platform3')" :value="3"></el-option>
          <el-option :label="$t('sms.platform1')" :value="4"></el-option>
    ...
    <el-table v-loading="state.dataListLoading" :data="state.dataList" border @selection-change="state.dataListSelectionChangeHandle" @sort-change="state.dataListSortChangeHandle" style="width: 100%">
      ...
      <el-table-column prop="platform" :label="$t('sms.platform')" header-align="center" align="center">
        <template v-slot="scope">
          <el-tag v-if="scope.row.platform === 1" size="small">云片</el-tag>
          <el-tag v-if="scope.row.platform === 5" size="small">塞班云</el-tag>
          <el-tag v-if="scope.row.platform === 4" size="small">{{ $t("sms.platform1") }}</el-tag>
